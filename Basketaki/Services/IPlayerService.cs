namespace Basketaki.Services
{
    public interface IPlayerService
    {
        ...
        Task<bool> CreateAsync(Player player);  // Create a new player, returns true if successful
        Task<bool> UpdateAsync(Player player);  // Update an existing player, returns true if successful
        Task<bool> DeleteAsync(int id);  // Delete a player by its ID, returns true if successful, false if not 
